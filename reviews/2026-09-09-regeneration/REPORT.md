# 公開データの再生成と監査（Issue #3）— 2026-09-09（最終再生成 2026-09-11）

対象: `pipeline/` で再生成した `public/M4_companies.json` / `public/M5_company_relations.json` / `public/evidence/`（生成日 2026-09-11）。
...
| --- | --- | --- |
| 関係（全体） | 81,445 | 83,479 |
| うち確定（confirmed） | — | 81,825 |
| うち要確認（needs_review） | — | 1,653 |
| うち過去（historical） | — | 1 |
| 非上場エンティティ | 59,780 | 57,937 |
| 数値・記号だけの企業ノード | 456 | **0** |
| 数値ノードに接続する関係 | 568 | **0** |
...
法人格を除いた本体が 1 文字以下の名称: 16 件（株式会社K、Z株式会社、株式会社創、(株)孝、(有)彩、有限会社志、株式会社晴、株式会社杏、有限会社欅、泉株式会社）。
...
| 名称 | 書類 | 文脈 | entity | 判定 |
| --- | --- | --- | --- | --- |
| 株式会社88 | S100W2DE | フェイスネットワーク(3489) の大株主 | ENT000100 | OK |
| 株式会社58 | S100XS2Y | エスネットワークス(5867) の大株主 | ENT000080 | OK |
| 株式会社28 | S100XHR6 | GLOE(9565) の連結子会社 | ENT000029 | OK |
...
| ケース | 期待 | 再生成後 | 判定 |
| --- | --- | --- | --- |
| 京成→OLC 関連会社（その他の関係会社） | confirmed | R0076586（confirmed・20.14%） | OK |
| OLC→京成 親子（旧 R0044475 の逆転） | absent | なし | OK |
| 京成→OLC 親子（Wikidata P749） | needs_review | R0076588（needs_review） | OK |
| SBG→ZOZO 親子 | confirmed | R0083377（confirmed・51.9%） | OK |
| ZOZO→SBG 親子（旧 R0003946 の逆転） | absent | なし | OK |
| ソニー→M3 関連会社 | confirmed | R0061503（confirmed・33.9%） | OK |
| M3→ソニー 親子（逆転） | absent | なし | OK |
| イオン→イオン北海道 親子 | confirmed | R0073327（confirmed・67.2%） | OK |
| 三菱商事→ライフ 関連会社 | confirmed | R0071718（confirmed・25.56%） | OK |
| 三菱商事→ライフ 親子（旧 100%） | absent | なし | OK |
| ライフ→三菱商事 親子（逆転） | absent | なし | OK |
| 日本車輌→JR東海 技術提携（根拠不足） | needs_review | R0064219（needs_review） | OK |
| アイシン→東邦瓦斯 共同研究（名寄せ） | confirmed | R0065411（confirmed） | OK |
...
旧 relation_id → 新 relation_id の対応表: `relation_id_map.json`（62,725 件対応、18,720 件対応なし）。
対応なしの主な内訳:

| 旧タイプ | 出所 | 件数 |
| --- | --- | --- |
| parent_subsidiary | edinet | 17,989 |
| ownership | edinet | 435 |
...
| parent_subsidiary | 44,675 | 36,602 |
...
| R0076182 | edinet/capital | 毎日コムネット → (株)ワークス・ジャパン | parent_subsidiary | correct | 関係会社の状況（S100WKTA、2025-05-31）連結子会社・議決権所有 87.0% と一致 |
| R0017138 | edinet/capital | 三好 稔美 → ファンペップ | ownership | correct | 大株主（個人）2.77% |
| R0066401 | edinet/capital | ＣＣＩグループ → 株式会社ハニー ビースポーツ | parent_subsidiary | correct | 関係会社の状況（S100YAOX、2026-03-31）連結子会社・議決権所有 100.00%（間接 100.00）と一致 |
...
| R0069314 | edinet/transaction | 旭化学工業 → マキタ | major_customer | correct | 主要な顧客 954,497千円（上場のマキタへ名寄せ） |
...
| R0068223 | ir_disclosure/alliance | 日本精密 → クロスフォー | business_alliance | correct | 業務提携の発表 |
...
| R0080121 | ir_disclosure/capital | テレビ東京ホールディングス → Minto | ownership | correct | 戦略的出資 |
...
| R0067540 | ir_disclosure/group | 島津製作所 → Plasmion GmbH | merger_acquisition | correct | 発行済株式の75%を取得し子会社化 |
...
| R0070260 | ir_disclosure/group | 丸紅 → TiAuto Investments Pty Ltd | merger_acquisition | correct | 公式公表で確認済み（agreed） |
...
| R0068018 | wikidata/capital | キヤノン → Canon Computer Systems | parent_subsidiary | correct_stale | Wikidata P355。現存しない可能性のある子会社で時点不明 |
| R0068333 | wikidata/capital | ブシロード → 新日本プロレス | parent_subsidiary | correct | Wikidata P355 |
| R0065541 | wikidata/capital | 本田技研工業 → ホンダマニュファクチャリングUK | parent_subsidiary | correct_stale | Wikidata P355。2021年に生産終了した拠点で時点不明 |
...
| R0077318 | wikidata/group | 名古屋鉄道 → 名鉄グループ | corporate_group | correct | Wikidata P463 |
| R0077029 | wikidata/group | 西日本鉄道 → 西鉄グループ | corporate_group | correct | Wikidata P463 |
| R0044862 | wikidata/group | ＧＭＯペパボ → ＧＭＯインターネットグループ | corporate_group | correct | Wikidata P463 |
| R0079046 | wikidata/group | ＡＮＡホールディングス → ANAグループ | corporate_group | correct | Wikidata P463 |
