# 実操作レビュー結果 2026-09-17

- 対象コミット: `8f9e7609c1d2864a4bcbd4c784f5d4ae1cf934a9`
...
| 負荷 | ドラッグ中の WebGL 描画呼び出し（約0.6秒） | 合格 | 490770 |
| 負荷 | 停止中の WebGL 描画呼び出し/秒 | 合格 | 0 |
| 3D | 描画休止中のリサイズ後に再描画される（ポインター操作なし） | 合格 | 366130 回 |
| 負荷 | リサイズ後も停止中は描画休止 | 合格 | 0 |
| 3D | ホイールで拡大（カメラが近づく） | 合格 | 5303 -> 4910 |
| 3D | ＋ボタンで拡大 | 合格 |  |
| 3D | 視点リセットで正面から全体を見る向きに戻る | 合格 | 0,0,5303 |
| 3D | 外側リリース後の再ドラッグで固着なし | 合格 |  |
| 3D | ホバー状態がノードに反映される | 合格 | hover=6349 (小森コーポレーション) |
| 3D | 回転後のクリックで見た目どおりの企業が開く | 合格 | 小森コーポレーション -> 小森コーポレーション |
| フィルタ | 全解除で空状態を表示 | 合格 |  |
| フィルタ | 初期状態に戻すで復帰 | 合格 |  |
...
| 3D | 円の内側だけがホバー対象（通常・ホバー拡大） | 合格 | ＧＭＯペパボ 中心(908,369) r=80 内側=ＧＭＯペパボ 四隅=なし/なし/なし/なし |
| 3D | 円の内側のクリックで同じ企業が開く（通常） | 合格 | ＧＭＯペパボ -> ＧＭＯペパボ |
| 3D | 当たり判定の確認前に視点が回転している | 合格 | 0,0,120 -> 31,16,115 |
| 3D | 円の内側だけがホバー対象（視点回転後） | 合格 | ＧＭＯペパボ 中心(914,365) r=84 内側=ＧＭＯペパボ 四隅=なし/なし/なし/なし |
| 3D | 円の内側のクリックで同じ企業が開く（視点回転後） | 合格 | ＧＭＯペパボ -> ＧＭＯペパボ |
