# 実操作レビュー結果 2026-09-19

- 対象コミット: `6a8ac13459cd2c2d777b5b2b816e1c2817cf55bd`
...
| 負荷 | 停止中の描画/秒 | 合格 | 0 |
| 2D | 描画休止中のリサイズ後に再描画される（ポインター操作なし） | 合格 | 9 回 |
...
| 2D | 配置計算中のズームが引き戻されない（倍率が単調に上がり、その後も保たれる） | 合格 | 1.180 1.247 1.318 1.393 1.473 1.557 1.646 1.739 1.839 / 2.5秒後 1.839 |
| フィルタ | グループのみ（低次数の企業だけ）でも表示中の企業が収まる | 合格 | k=0.341 中心(127,20) -> k=20.000 中心(0,0) / 2 表示中の企業 1 表示中の関係 上場企業間 4,996件中 |
...
| 往復 | 中心企業にホバーしても線の種別ラベルが増えない（崩れない） | 合格 | 線 119 本 / 種別ラベル 0 -> 0 |
...
| キーボード | 左右矢印キーで表示位置を水平に移動 | 合格 | k=0.341 中心(128,20) -> k=0.341 中心(362,20) |
| キーボード | 上下矢印キーで表示位置を垂直に移動 | 合格 | k=0.341 中心(362,20) -> k=0.341 中心(362,-215) |
| キーボード | −キーで縮小（倍率が下がる） | 合格 | 0.341 -> 0.244 |
| キーボード | 0キーで全体を表示（初期の倍率・位置に戻る） | 合格 | k=0.341 中心(127,20) / 初期 k=0.341 中心(128,20) |
| 200% | 横スクロールなし | 合格 |  |
| 200% | 最小関係数スライダーにスクロールで到達 | 合格 |  |
| 200% | 一覧の操作部品が表示される | 合格 |  |
| タッチ | 横スクロールなし | 合格 |  |
| タッチ | 向き変更後に再描画される（ポインター操作なし） | 合格 | 7 回 |
