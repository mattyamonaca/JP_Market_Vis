...
各関係の `evidence` に出所・出所種別（一次開示／二次情報／LLM抽出）・基準日・公表日・取得日・原本URL・書類ID・抽出根拠・引用を保持し、詳細画面で確認できます。基準日が不明な場合は「不明」と表示します。エビデンス全文は `public/evidence/` の分割ファイルにあり、詳細を開いたときに読み込みます。関係には `status`（確定／要確認／過去）があり、全体マップ・ランキング・統計は確定関係だけを数えます。「一次開示」は出所の種別で、抽出が正しいことを意味しません。人手で原本と照合した関係だけに「原本で検証済み」が付きます。出所別統計はエビデンス件数です。ソースコードのライセンスはリポジトリの [LICENSE](LICENSE) を参照してください。収録データ・引用元には各提供元の利用条件が適用されます。
