...
**東証プライム・スタンダード・グロースの内国株式 3,734 社を対象に、確定関係 88,813 件（上場企業間は約 11,000 件）を収録しています。** 各関係には出所（有価証券報告書の書類 ID・Wikidata の項目・公表資料の URL）、基準日、構造化した抽出項目を付け、画面から原本へたどれます。
...
HTML・JavaScript・CSS・JSON だけで動作し、外部 API・バックエンド・API キーは不要です。初回データは gzip 転送で約 2.7MB（展開後 約 43MB）、抽出項目・出典・比率履歴の詳細（約 49.5MB）は分割ファイルにあり、詳細を開いたときに該当分だけ読み込みます。
...
| `public/evidence/<shard>.json` | 関係 ID ごとの構造化抽出情報と出典（基準日・原本 URL・書類 ID・資料内の位置・役職・契約年月・出資比率の内訳と履歴。原文引用なし） |
