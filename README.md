...
円をドラッグすると、そのノードを動かして重なった企業を確認できます。離した場所に固定され、線も追従します。「ノードの固定を解除」で自動配置に戻せます。カテゴリや最小関係数を変更した場合も固定は解除されます。背景のドラッグはマップ全体の移動、スクロール／ピンチは拡大縮小です。円をクリック（動かさずに離す）すると、その企業の関係グラフへ進みます。ドラッグの終了では開きません。
